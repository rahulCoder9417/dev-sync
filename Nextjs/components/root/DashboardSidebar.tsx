...
const DashboardSidebarComponent = () => {
  ...
  const renderedRecentProjects = useMemo(
    () =>
      recentProjects.map((project) => (
        <Link key={project.id} href={`/projects/${project.id}`} prefetch={true}>
          <div className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-700/50 transition-colors cursor-pointer">
  ...
  return (
    <>
      ...
      <div
        ...
        } overflow-hidden border-r  border-primary hidden md:flex flex-col bg-[var(--bg-secondary)]`}
      ...
      >
        <div className="p-4 flex flex-col flex-1">
          <div className="flex justify-between items-center mb-4">
            ...
            <button className="text-white cursor-pointer" onClick={() => setIsSidebarOpen(false)}>
              <CircleX />
            </button>
          </div>

          <nav className="space-y-2">{renderedSidebarItems}</nav>
          ...
          <div className="mt-8 flex-1 overflow-auto">
            ...
            <div className="space-y-2">{renderedRecentProjects}</div>
          </div>
        </div>
      </div>
    </>
