  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      let userAuth,
        ...
        project,
        folder = [],
        fileContents: { path: string; content: string | null; type: string }[] =
          [];
      ...
      try {
        ...
        try {
          userAuth = decrypt(formData.get("userAuth")?.toString() || "");
        } catch (e) {
          send({ step: 1, status: "error", message: `Invalid token` });
          controller.close();
          return;
        }
        ...
        if (!userAuth || !ownerName || !repoName) {
          throw new Error(
            "Missing GitHub auth token,and git repo details - 400"
          );
        }
      ...
      try {
        ...
        const extractedFiles = unzipSync(
          zipUint8Array as Uint8Array<ArrayBuffer>
        ); // key: path, value: Uint8Array
        ...
        for (const i in extractedFiles) {
          ...
          const filename = i.startsWith(rootPrefix)
            ? i.slice(rootPrefix.length)
            : i;
          if (isFolder) {
        ...
        send({
          step: 4,
          status: "done",
          message: "Repo Procssed Going To mske project",
        });
      } catch (err) {
        send({
          step: 4,
      ...
      try {
        ...
        for (const i of folder) {
          ...
          const parentPath =
            pathPart.slice(0, -1).length === 0
              ? null
              : pathPart.slice(0, -1).join("/") + "/";
